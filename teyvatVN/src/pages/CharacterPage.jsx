...
export default function CharacterPage() {
  ...
  const handleSelect = (char) => {
    const alreadySelected = selectedCharacters.some(c => c.name === char.name);
    if (alreadySelected) {
  ...
  return (
    <div className="bg-white min-h-screen flex flex-col justify-between text-gray-900">
      {/* Header */}
      ...
      <main className="w-full max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between gap-4 mb-6 flex-wrap">
          ...
          <input
            type="text"
            placeholder="Search"
            className="w-full max-w-xs px-4 py-2 border rounded-full bg-white text-sm border-[#C7C3C3] placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-400"
          />
  ...
  {characters.map((char) => (
    <div
      ...
      className={`cursor-pointer border border-[#C7C3C3] rounded-md overflow-hidden transition-transform hover:scale-105 shadow-sm ${
        selectedCharacters.some(c => c.name === char.name)
    ...
    >
      ...
      />
      <div className="bg-[#A7B7E4] text-center py-1 font-medium text-sm">
        {char.name}
      </div>
    </div>
  ))}
      ...
      <footer className="bg-[#1c1b3a] text-white text-xs p-4 text-center">
        ...
        <br />
        Quill pen SVG by <a href="https://thenounproject.com/icon/quill-pen-3147689/" target="_blank" className="underline">Noun Project</a> (CC BY 3.0).
      </footer>
    </div>
  );
}
